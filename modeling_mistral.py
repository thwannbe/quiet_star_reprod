...
class MistralForCausalLM(MistralPreTrainedModel):
    ...
    def infer(
        ...
        notokendump = False
        if not notokendump:
            with open('./token_ids.dump', 'a') as f:
                f.write(f'hello size = {input_ids.shape}\n')
        
        def token_ids_dump(ids: torch.Tensor) -> None:
            if not notokendump:
                with open('./token_ids.dump', 'a') as f:
                    f.write(f'{ids.tolist()}\n')
        
        batch_size, seq_len = input_ids.shape
        dynamic_thought = False
        dynamic_conf_threshold = 0.5
        ...
        #     token_ids_dump(input_ids[0]) # only first batch
        # else:
        #     token_ids_dump(input_ids) # one batch input
        ...
        if dynamic_thought:
            ...
            next_token_logits = F.softmax(logits, dim=-1)
            next_token_confidence = torch.max(next_token_logits)
            # print(next_token_confidence)

            if torch.all(next_token_confidence > dynamic_conf_threshold):
                # print("No need thought")
    ...
    def forward(
        ...
        if not nolossdump:
            with open('./loss_analysis.dump', 'a') as f:
                f.write(f'hello new query\n')
        
        def myprint(s:str) -> None:
            if not nolog:
                print(f"Hello OSW: {s}")
        ...
        orig_loss_probs = None
        orig_loss_loss = None
        after_loss_probs = None
        after_loss_loss = None
        
        myprint(f"input_ids = {input_ids}")
        myprint(f"input_ids.size = {input_ids.shape}")
        myprint(f"input_ids[0].decode = {self.tokenizer.decode(input_ids[0])}")
        
        token_ids_dump(input_ids[0])
        ...
        if labels is not None:
            myprint(f"labels = {labels}")
            myprint(f"labels.shape = {labels.shape}")

        self.tokenizer_has_start_thought_token = True
        self.tokenizer_has_end_thought_token = True
        if self.start_token_id is None:
            myprint(f"At init, start_token_id initialize")
            ...
            myprint(f"start_embedding = {self.start_embedding}")
            myprint(f"start_embedding[0] = {self.start_embedding[0]}")
            myprint(f"start_embedding[0].shape = {self.start_embedding[0].shape}")
        if self.end_token_id is None:
            myprint(f"At init, end_token_id initialize")
            ...
            myprint(f"end_embedding = {self.end_embedding}")
            myprint(f"end_embedding[0] = {self.end_embedding[0]}")
            myprint(f"end_embedding[0].shape = {self.end_embedding[0].shape}")

        if not self.rm_initialized and (self.n_ahead > 1 or not self.base_original_mode):
            myprint(f"rm initialization @@@@@@@@@@@@@@@@@@@@@@@@@@@@")
        ...
        myprint(f"start_embedding = {start_embedding}")
        myprint(f"start_embedding.shape = {start_embedding.shape}")
        myprint(f"end_embedding = {end_embedding}")
        myprint(f"end_embedding.shape = {end_embedding.shape}")
        
        # # decoder outputs consists of (dec_features, layer_state, dec_hidden, dec_attn)
        fwd_iters = 1 if self.original_mode else self.n_ahead + self.n_ahead_talk - 1
        for ahead_idx in range(fwd_iters):
            myprint(f"ahead_idx = {ahead_idx}, fwd_iters = {fwd_iters}")
            past_key_values_length = 0
            if past_key_values is not None:
                use_legacy_cache = not isinstance(past_key_values, Cache)
                if use_legacy_cache:
            ...
            myprint(f"position_ids = {position_ids}")
            myprint(f"position_ids.shape = {position_ids.shape}")

            if inputs_embeds is None:
                myprint(f"Only called at first time @@@@@@@@@@@@@@@@@@@@@")
            ...
            myprint(f"inputs_embeds[0] = {inputs_embeds[0]}")
            myprint(f"inputs_embeds.shape = {inputs_embeds.shape}")
            
            myprint(f"attention_mask = {attention_mask}")
            myprint(f"attention_mask.shape = {attention_mask.shape}")
            ...
            # myprint(f"outputs.shape = {outputs.shape}")
            myprint(f"hidden_states = {hidden_states}, hidden_states.shape = {hidden_states.shape}")
            myprint(f"rm_logits = {rm_logits}")
            myprint(f"cur_rm_tokens = {cur_rm_tokens}")

            if ahead_idx == 0:
                myprint(f"First thought step @@@@@@@@@@@@@@@@@@@@")
                ...
                myprint(f"base_hidden_states = {base_hidden_states}")
                myprint(f"base_hidden_states.shape = {base_hidden_states.shape}")
                myprint(f"initial_loss_logits = {initial_loss_logits}")
                myprint(f"initial_loss_logits.shape = {initial_loss_logits.shape}")
                ...
                if not nolossdump:
                    ...
                    shift_probs = shift_probs.detach().cpu()
                    shift_labels = shift_labels.detach().cpu()
                    
                    orig_loss_probs = shift_probs[torch.arange(shift_labels.size(0)),shift_labels].detach().cpu()
                    myprint(f"orig_loss_probs = {orig_loss_probs}")
                    myprint(f"orig_loss_probs.shape = {orig_loss_probs.shape}")
                    orig_loss_loss = loss.clone().detach().cpu()
                    myprint(f"orig_loss_loss = {orig_loss_loss}")
                    myprint(f"orig_loss_loss.shape = {orig_loss_loss.shape}")
            else:
                myprint(f"More than Second step @@@@@@@@@@@@@@@@@@@@")
                talk_hidden_states = hidden_states
                if self.merged_lm_and_talk_heads:
                    myprint(f"Never reach here @@@@@@@@@@@@@@@@@@@@")
                    exit(1)
                    ...
                    myprint(f"(weight) residual_logits = {residual_logits}")
                    myprint(f"(weight) residual_logits.shape = {residual_logits.shape}")
                    ...
                    if self.use_weighted_talk_head:
                        ...
                        residual_logits = apply_head(self.lm_head, residual_logits, detach=self.optimize_lm_head_only_at_start)
                        myprint(f"(real) residual_logits = {residual_logits}")
                        myprint(f"(real) residual_logits.shape = {residual_logits.shape}")
            ...
            if not attempted or self.comparison_mode:
                ...
                myprint(f"rm_logits = {rm_logits}")
                myprint(f"rm_logits.shape = {rm_logits.shape}")
                ...
                myprint(f"probabilities_2d = {probabilities_2d}")
                myprint(f"probabilities_2d.shape = {probabilities_2d.shape}")
                ...
                if ahead_idx == 0 and self.use_start_thought_token:
                    ...
                    myprint(f"think end here @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
                    override_token = self.end_token_id
                else:
                    override_token = None
                myprint(f"override_token = {override_token}")
                if override_token is not None and self.n_ahead > 1:
                    myprint(f"Start or end thought token here @@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
                    ...
                    if labels is not None:  # we're in the talk phase
                        ...
                        myprint(f"new_rm_tokens = {new_rm_tokens}")
                        myprint(f"new_rm_tokens.shape = {new_rm_tokens.shape}")
                        ...
                        if not nolossdump and ahead_idx == self.n_ahead - 1: # only dump for first talk
                            shift_logits = logits[..., :-1, :].contiguous()
                            shift_labels = labels[..., 1:].contiguous()
                            # Flatten the tokens
                            loss_fct = CrossEntropyLoss(reduction="none")
                            ...
                            after_loss_probs = shift_probs[torch.arange(shift_labels.size(0)),shift_labels].detach().cpu()
                            myprint(f"after_loss_probs = {after_loss_probs}")
                            myprint(f"after_loss_probs.shape = {after_loss_probs.shape}")
                            after_loss_loss = loss.clone().detach().cpu()
                            myprint(f"orig_loss_loss = {after_loss_loss}")
                            myprint(f"orig_loss_loss.shape = {after_loss_loss.shape}")
                    else:
                        continue
                ...
                next_sampled_token_ids = probabilities_2d.argmax(dim=-1).detach().cpu()
                myprint(f"next_sampled_token_ids.decode = {self.tokenizer.decode(next_sampled_token_ids)}")
                token_ids_dump(next_sampled_token_ids)
                sampled_token_history.append(probabilities_2d.argmax(dim=-1).detach().cpu())
                myprint(f"sampled_token_history = {sampled_token_history}")
                ...
                contains_thought = contains_start or contains_end
                myprint(f"contains_start,contains_end = {contains_start,contains_end}")
                ...
                if not contains_thought:
                    ...
                    if self.use_reparam_for_thought_embeddings:
                        ...
                        myprint(f"inputs_embeds = {inputs_embeds}")
                        myprint(f"inputs_embeds.shape = {inputs_embeds.shape}")
                inputs_embeds = inputs_embeds.view(probabilities.size(0), probabilities.size(1), -1).to(self.model.embed_tokens.weight.dtype)
                myprint(f"inputs_embeds = {inputs_embeds}")
                myprint(f"inputs_embeds.shape = {inputs_embeds.shape}")

                myprint(f"Update attention_mask for the next iteration")
                if len(attention_mask.shape) == 2:
                    ...
                    if self.use_upper_triangular:
                        ...
                        myprint(f"original_attention = {original_attention}")
                        myprint(f"original_attention.shape = {original_attention.shape}")
                        ...
                        myprint(f"new_attention = {new_attention}")
                        myprint(f"new_attention.shape = {new_attention.shape}")
                ...
                if labels is not None and (self.n_ahead > 1 or not self.base_original_mode):
                    ...
                    myprint(f"shift_idx = {shift_idx}")
                    shift_logits = loss_logits[..., :-shift_idx, :].contiguous()
                    shift_labels = labels[..., shift_idx:].contiguous()
                    myprint(f"shift_logits = {shift_logits}")
                    myprint(f"shift_logits.shape = {shift_logits.shape}")
                    myprint(f"shift_labels = {shift_labels}")
                    myprint(f"shift_labels.shape = {shift_labels.shape}")
                    # Flatten the tokens
                    loss_fct = CrossEntropyLoss(reduction="none")
                    shift_logits = shift_logits.view(-1, self.config.vocab_size)
                    ...
                    myprint(f"unreduced_loss = {unreduced_loss}")
                    myprint(f"unreduced_loss.shape = {unreduced_loss.shape}")
                    loss_list.append(unreduced_loss)
                    myprint(f"loss_list = {loss_list}")


                    if self.use_policy_loss and ahead_idx > 0 and (ahead_idx > 1 or not self.use_start_thought_token):
                        myprint(f"From third step, RL starts @@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
                        ...
                        myprint(f"original_dqn_reward = {original_dqn_reward}")
                        myprint(f"original_dqn_reward.shape = {original_dqn_reward.shape}")
                                
                        if not did_skip_sampling:
                            myprint(f"previous step wasn't start/end thought token @@@@@@@@@@@@@@@@@@@@@@@")
                            nonzero_indices = prev_probabilities_2d.nonzero()
                            myprint(f"nonzero_indices = {nonzero_indices}")
                            myprint(f"nonzero_indices.shape = {nonzero_indices.shape}")
                            ...
                            myprint(f"action_loglikelihoods = {action_loglikelihoods}")
                            myprint(f"action_loglikelihoods.shape = {action_loglikelihoods.shape}")
                            action_loglikelihoods_2d = action_loglikelihoods.reshape(batch_size, -1)[:, :-1 - shift_amount]
                            myprint(f"action_loglikelihoods_2d = {action_loglikelihoods_2d}")
                            myprint(f"action_loglikelihoods_2d.shape = {action_loglikelihoods_2d.shape}")
                        ...
                        myprint(f"policy_reward = {policy_reward}")
                        myprint(f"policy_reward.shape = {policy_reward.shape}")
                    
                    if self.use_policy_loss and ahead_idx == self.n_ahead + self.n_ahead_talk - 2:
                        myprint(f"For the last step @@@@@@@@@@@@@@@@@@@@@@@@")
        ...
        myprint(f"loss aggregation start @@@@@@@@@@@@@@@@@@@@@")
        if loss_list:
            if self.first_and_last_mode:
                myprint(f"Always first and last mode @@@@@@@@@@@@@@@@@@@@@")
                loss = sum(
        ...
        myprint(f"final_loss_logit: {logits}")
        myprint(f"final_loss_logit.shape: {logits.shape}")
        ...
        if not nolossdump:
            loss_dump(f"orig_loss_probs: {orig_loss_probs.tolist()}")
            loss_dump(f"orig_loss_loss: {orig_loss_loss.tolist()}")
            loss_dump(f"after_loss_probs: {after_loss_probs.tolist()}")
            loss_dump(f"after_loss_loss: {after_loss_loss.tolist()}")
